...
/**
 * This is the mock synthesizer class.
 */
public class MockSynthesizer implements Synthesizer {
   ...
   * synthesizer actually produces the corresponding result. <p> Although the
   * latency is expressed in microseconds, a synthesizer's actual measured
   * delay may vary over a wider range than this resolution suggests.  For
   * example, a synthesizer might have a worst-case delay of a few milliseconds
   * or more.
   *
   ...
   * receives the MIDI messages sent on that channel number. <p> The MIDI 1.0
   * specification provides for 16 channels, so this method returns an array of
   * at least 16 elements.  However, if this synthesizer doesn't make use of
   * all 16 channels, some of the elements of the array might be
   * <code>null</code>, so you should check each element before using it.
   *
   ...
   * this class of <code>Synthesizer</code> does not provide voice information,
   * the returned array will always be of length 0.  Otherwise, its length is
   ...
   * instruments from the specified soundbank. If the soundbank is unsupported,
   * any attempts to load instruments from it will result in an
   ...
   * loaded into the patch location specified by its <code>Patch</code> object,
   * so that if a program-change message is received (or has been received)
   * that causes that patch to be selected, subsequent notes will be played
   * using the sound of <code>instrument</code>.  If the specified instrument
   * is already loaded, this method does nothing and returns <code>true</code>.
   * <p> The instrument must be part of a soundbank that this
   ...
   * loaded (for example, if the synthesizer has insufficient memory to load
   * it)
   * @throws IllegalArgumentException if this <code>Synthesizer</code> doesn't
   *                                  support the specified instrument's
   *                                  soundbank
   ...
   * instrument <code>from</code>.<br> For example, if <code>from</code> was
   * located at bank number 2, program number 11, remapping causes that bank
   * and program location to be occupied instead by <code>to</code>.<br> If the
   * function succeeds,  instrument <code>from</code> is unloaded. <p>To cancel
   * the remapping reload instrument <code>from</code> by invoking one of
   * {@link #loadInstrument}, {@link #loadInstruments} or {@link
   * #loadAllInstruments}.
   *
   * @param from the <code>Instrument</code> object to be replaced
   * @param to   the <code>Instrument</code> object to be used in place of the
   ...
   * @throws IllegalArgumentException if instrument <code>from</code> or
   *                                  instrument <code>to</code> aren't
   *                                  supported by synthesizer or if instrument
   *                                  <code>to</code> is not loaded
   * @throws NullPointerException     if <code>from</code> or <code>to</code>
   ...
   * a default soundbank provided with the synthesizer, etc. <p> Note that you
   * don't use this method  to find out which instruments are currently loaded
   * onto the synthesizer; for that purpose, you use <code>getLoadedInstruments()</code>.
   * Nor does the method indicate all the instruments that can be loaded onto
   * the synthesizer; it only indicates the subset that come with the
   * synthesizer. To learn whether another instrument can be loaded, you can
   * invoke <code>isSoundbankSupported()</code>, and if the instrument's
   ...
   *
   * @param soundbank the <code>Soundbank</code> whose are instruments are to
   *                  be loaded
   * @return <code>true</code> if the instruments are all successfully loaded
   * (or already had been), <code>false</code> if any instrument could not be
   * loaded (for example, if the <code>Synthesizer</code> had insufficient
   * memory)
   ...
   * @throws IllegalArgumentException thrown if the soundbank is not
   *                                  supported.
   * @see #isSoundbankSupported
   * @see #unloadInstrument
   * @see #unloadInstruments
   ...
   * location.
   *
   * @param soundbank the <code>Soundbank</code> containing the instruments to
   *                  load
   ...
   *
   * <p>An application opening a device explicitly with this call has to close
   * the device by calling {@link #close}. This is necessary to release system
   * resources and allow applications to exit cleanly.
   *
   * <p> Note that some devices, once closed, cannot be reopened.  Attempts to
   * reopen such a device will always result in a MidiUnavailableException.
   *
   * @throws MidiUnavailableException thrown if the device cannot be opened due
   *                                  to resource restrictions.
   * @throws SecurityException        thrown if the device cannot be opened due
   ...
   *
   * <p>All <code>Receiver</code> and <code>Transmitter</code> instances open
   ...
   * supports time-stamps, it should start counting at 0 when the device is
   * opened and continue incrementing its time-stamp in microseconds until the
   * device is closed. If it does not support time-stamps, it should always
   * return -1.
   *
  ...
  public long getMicrosecondPosition() {
    log.append("Getting microsecond position...");
    log.append("\n");
    return 0;
  }
   ...
   * finished using it.
   *
   * <p>Usually the returned receiver implements the {@code MidiDeviceReceiver}
   * interface.
   *
   * <p>Obtaining a <code>Receiver</code> with this method does not open the
   ...
   * close the device. It has to be closed explicitly by calling {@link
   * #close}.
   *
   * @return a receiver for the device.
   * @throws MidiUnavailableException thrown if a receiver is not available due
   *                                  to resource restrictions
   * @see Receiver#close()
   */
  @Override
   ...
   *
   * <p>Usually the returned receivers implement the {@code MidiDeviceReceiver}
   * interface.
   *
   ...
   *
   * <p>Usually the returned transmitter implements the {@code
   * MidiDeviceTransmitter} interface.
   *
   * <p>Obtaining a <code>Transmitter</code> with this method does not open the
   * device. To be able to use the device, it has to be opened explicitly by
   * calling {@link #open}. Also, closing the <code>Transmitter</code> does not
   * close the device. It has to be closed explicitly by calling {@link
   * #close}.
   *
   ...
   *
   * <p>Usually the returned transmitters implement the {@code
   * MidiDeviceTransmitter} interface.
   *
